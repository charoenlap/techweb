<?php
	function xlsx($title='',$data=array()){
		require DOCUMENT_ROOT.'/system/lib/PHPExcel-1.8/Classes/PHPExcel.php';
		...
		$objXlsx = $objPHPExcel->setActiveSheetIndex(0);
		$column = 1;
		foreach($data as $key => $val){
			$letterAscii = 65;
			foreach($val as $v){
				$letter = chr($letterAscii);
				// echo $letter.$column.'='.$v.' > ';
				$text_column = $letter.$column;
				$objXlsx->setCellValue($text_column, $v);
				$letterAscii++;
	        }
	        $column++;
		}
		...
		header('Content-Disposition: attachment;filename="'.$title.'_'.time().'.xlsx"');
	...
	function l($index=''){
		...
		// var_dump($lang);
		$route = get('route');
		if(isset($lang[$route][$index])){
			$result = $lang[$route][$index];
		}else{
			$result = $index;
		}
		return $result;
	...
	function sendmailSmtp($to_email,$msg,$subject="",$email_bcc=""){
